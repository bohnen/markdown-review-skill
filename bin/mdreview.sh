...
usage="使い方: mdreview.sh markdownlint [--fix] <file...> | mdreview.sh textlint <technical|blog|book> [--fix] <file...>"
...
case "$TOOL" in
  markdownlint)
    ...
    ;;
  textlint)
    [ $# -ge 1 ] || die "textlint には種別 <technical|blog|book> が必要です。"
    TYPE="$1"; shift
    case "$TYPE" in
      technical|blog|book) : ;;
      *) die "未知の種別: $TYPE（technical|blog|book）" ;;
    esac
    [ $# -ge 1 ] || die "$usage"
