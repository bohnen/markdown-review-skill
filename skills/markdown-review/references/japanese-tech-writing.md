...
`markdown-review` スキルの文章規範レビューで参照する規範集。日本語の技術文書（書籍の章、記事、解説文）やブログを推敲するとき、以下に照らして指摘する。機械 lint（markdownlint / textlint）では拾えない、段落構成・論証・演出・冗長などの観点を扱う。

技術文書は標準（である調の許容）、ブログは「ですます」で、いずれもこの規範を適用する。指摘は提案として返し、事実・主張の書き換えを勝手に確定しない。
